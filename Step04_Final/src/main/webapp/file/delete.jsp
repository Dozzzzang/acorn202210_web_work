...
<%
	...
	//삭제 작업을 하기 전에 로그인된 아이디와 글 작성자가 같은지 확인해서 일치할때만 삭제하도록 한다.
	String id=(String)session.getAttribute("id");
	//만일 글 작성자와 로그인된 아이디가 같지 않으면
	if(!dto.getWriter().equals(id)){
		//금지된 요청이라고 에러페이지를 응답한다.
		response.sendError(HttpServletResponse.SC_FORBIDDEN, "남의 파일 지우면 혼난다!");
		return; //메소드를 여기서 종료하기(아래의 코드가 실행 되지 않도록)
	}
